...
const updateScore = ({ userScore, attempts, timeFormatted }) => {
  ...
  const hadPreviousRecord = userScore.attempts > 0;

  const fewerAttempts =
    !hadPreviousRecord || currentAttempts < userScore.attempts;
...
const printWinMessage = ({
  ...
  if (hadPreviousRecord && fewerAttempts) {
    console.log(
  ...
  if (hadPreviousRecord && fasterTime) {
    console.log(
